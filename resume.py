...
class Resume(CV):
	...
	class Document:
		...
		def flatten_dict(d):
		    """
		    Convert a nested dictionary to a flattened dictionary to make it hashable.
		    """
		    items = []
		    for key, value in d.items():
		        if isinstance(value, dict):
		            items.extend((f"{key}.{k}", v) for k, v in flatten_dict(value).items())
		        else:
		...
		def cached(func):
		    def memoize(self, *args, **kwargs):
		        ...
		        key = (
		        	func.__name__,
		        	tuple(args),
		        	frozenset(self.flatten_dict(kwargs)))
